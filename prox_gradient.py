...
def train_mse_pytorch(X, Y, bias, dest_dir, tol=1e-6, lam=1e-2, mu=1e-6, pre_trained_V=None, wandb=None):
    k = X.shape[0]
    n = X.shape[1]
    d = Y.shape[0]
    device = 'cuda:1'
    ...
    while keep_going:
        ...
        if wandb:
            wandb.log({'active':active_curr,'gl':gl_curr,'mse':mse_curr})
        ...
        if iters % 50000 == 0:
            ...
            np.save(os.path.join(dest_dir, 'V_new_iter_{}.npy'.format(iters)), V_k.detach().cpu().numpy())
        iters += 1
        gl_prev = gl_curr
        mse_prev = mse_curr
